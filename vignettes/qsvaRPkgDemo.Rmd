...
First we transform the data to a long format.
...
Then we extract the data we're interested in from the colData of the `rse_tx` object. 
...
Finally we join the `tpm` with the metadata. 
...
And calculate the mean tpm for each Dx and region. 
...
Let's have a look at the boxplots.
